# Facebook Sales Posts — Ready to Publish

## POST 1: PIN THIS — Introduction + CTA
**Type:** Text + Photo (before/after cabinet painting)

```
Handy & Friend — your local LA handyman.

What we do:
- Kitchen cabinet painting (spray finish from $155/door)
- TV mounting ($165, hidden wires $250)
- Interior painting (from $3/sq ft)
- Furniture assembly (from $150)
- Art & mirror hanging ($175)
- Flooring (LVP from $3.50/sq ft)

Insured. Upfront pricing. No hidden fees.
We respond within 1 hour, Mon-Sat 8am-8pm PT.

Get a free quote:
Call/text: (213) 361-1700
WhatsApp: wa.me/12133611700
Online: handyandfriend.com

Message us right here on Facebook — Alex will give you an instant estimate.
```

**CTA Button:** Send Message
**Pin this post**

---

## POST 2: PIN THIS — Combo Offer

```
This month: Book 2+ services together and save 20%.

Popular combos:
TV mount + art hanging = $340 → $272 (save $68)
Cabinet painting + interior walls = custom quote, 20% off combo
Furniture assembly + art hanging = $325 → $260 (save $65)

No hidden fees. Labor pricing is upfront.

Text (213) 361-1700 or message us here for your combo quote.

handyandfriend.com
```

**CTA Button:** Send Message
**Pin this post**

---

## POST 3: Social Proof — Recent Job

```
Just finished a 20-door cabinet painting in [Hollywood/Silver Lake/etc].

Full spray finish with premium paint — looks factory-new.
Total time: 4 days. Cost: less than 1/3 of cabinet replacement.

Before → After photos in the comments.

Need your cabinets refreshed? Text (213) 361-1700 or message us here.

#LosAngeles #KitchenMakeover #CabinetPainting #HandyAndFriend
```

**CTA Button:** Send Message
**Attach:** Before/after photos from completed job

---

## POST 4: TV Mounting Tip (Engagement)

```
Thinking about mounting your new TV? Here's what most people get wrong:

1. Drywall anchors alone won't hold 50+ lbs — always use studs
2. Center of TV should be at eye level from your couch (42-48")
3. In-wall cable concealment looks 10x cleaner than cable covers

We mount TVs daily in LA. Standard mount $165, hidden wires $250.

Available same day most weeks.

Call (213) 361-1700 or message us here.

#TVMounting #LosAngeles #HomeImprovement
```

---

## POST 5: Seasonal / Urgency

```
Spring is here — perfect time for a home refresh.

Popular spring projects:
Interior painting (from $3/sq ft — you pick the color)
Cabinet repaint (spray finish from $155/door)
New flooring (LVP from $3.50/sq ft)

Book this week and get 20% off any combo of 2+ services.

Free estimate: (213) 361-1700
Or message us right here — Alex will calculate your price.

handyandfriend.com
```

---

## POSTING SCHEDULE

| Week | Post | Type |
|---|---|---|
| Week 1 | Post 1 (pin) + Post 2 (pin) + Post 3 | Launch |
| Week 2 | Post 4 + Post 5 | Engagement + Urgency |
| Week 3+ | 2 posts/week | Rotate: job photos, tips, offers |

## RULES
- Every post ends with CTA (phone + "message us here")
- Include 1+ real photo when possible
- Reply to all comments within 2 hours
- Reply to all DMs within 5 minutes (Alex handles this automatically once connected)

## STOP-LOSS
- 2 weeks zero engagement → reduce to 1 post/week
- Posts performing well but zero inquiries → adjust CTA clarity
